...
public class ShowMatchStats{
    ...
    public static String showStats(Match match) {
        ...
        if (firstInningScore > secondInningScore) {
            ...
            result += String.format("Team %s won by %d wickets%n", match.getInnings().get(1).getBattingTeam().getName(), match.getInnings().get(1).getBattingTeam().getTeamSize() - ((TeamStats) match.getInnings().get(1).getBattingTeam().getTeamStats()).getTotalWickets());
